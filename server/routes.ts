...
import { transcribeAudio, extractActionItems, calculateAccuracyRate } from "./services/openai";
...
const upload = multer({
  ...
  fileFilter: (req: Express.Request, file: Express.Multer.File, cb: multer.FileFilterCallback) => {
    ...
    if (allowedTypes.includes(ext)) {
      ...
      cb(new Error('Invalid file type. Only MP3, MP4, WAV, and M4A files are allowed.'));
    }
  },
});
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/meetings/upload", upload.single('audioFile'), async (req: MulterRequest, res) => {
    try {
      if (!req.file) {
      ...
      res.json({ meeting, message: "Upload successful, transcription started" });
    } catch (error) {
      ...
      const results = await Promise.all(
        tests.map(async (test) => {
          ...
          const avgAccuracy = testResults.length > 0 
            ? Math.round(testResults.reduce((sum, r) => sum + (r.accuracyRate || 0), 0) / testResults.length)
            : 0;
          const avgProcessingTime = testResults.length > 0
            ? Math.round(testResults.reduce((sum, r) => sum + (r.processingTime || 0), 0) / testResults.length / 60 * 10) / 10
            : 0;
          const avgActionItems = testResults.length > 0
            ? Math.round(testResults.reduce((sum, r) => sum + (r.actionItemsFound || 0), 0) / testResults.length)
            : 0;
  ...
  app.get("/api/stats", async (req, res) => {
    try {
      ...
      const monthlyMeetings = meetings.filter(m => new Date(m.createdAt) >= thisMonth);
      const monthlyActionItems = actionItems.filter(ai => new Date(ai.createdAt) >= thisMonth);
...
async function processTranscription(meetingId: string, filePath: string, abTestGroup: string) {
  try {
    ...
    const accuracyRate = await calculateAccuracyRate(actionItems, transcriptionResult.text);
    await storage.createTestResult({
      testId: test.id,
      meetingId,
      accuracyRate,
      processingTime,
      actionItemsFound: Math.round((actionItems.length / transcriptionResult.text.length) * 1000), // Items per 1000 characters
    });
