import { type Meeting, type InsertMeeting, type ActionItem, type InsertActionItem, type AbTest, type InsertAbTest, type TestResult, type InsertTestResult } from "@shared/schema";
...
export class MemStorage implements IStorage {
    ...
    const defaultTests = [
      {
        ...
        model: "default",
        prompt: "Extract action items from this meeting transcript. Focus on tasks, assignments, and deadlines.",
        isActive: true,
      },
      {
        ...
        model: "enhanced",
        prompt: "Carefully analyze this meeting transcript and extract all action items, including implicit tasks, assignments with specific people, deadlines, and follow-up items. Format as JSON with text, assignee, priority, and dueDate fields.",
        isActive: true,
      },
      {
        ...
        model: "speed",
        prompt: "Quickly identify key action items and tasks from this transcript.",
        isActive: true,
      },
    ];
  ...
  async searchMeetings(query: string): Promise<Meeting[]> {
    ...
    return meetings.filter(meeting => 
      ...
      (meeting.transcriptionText && meeting.transcriptionText.toLowerCase().includes(query.toLowerCase()))
    );
  }
  ...
  async getTestResultsByMeeting(meetingId: string): Promise<TestResult[]> {
    return Array.from(this.testResults.values())
      .filter(result => result.meetingId === meetingId);
  }
}
